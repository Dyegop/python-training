...
-With asyncio you can create asynchronous routines are able to “pause” while waiting on their ultimate result and
...
-Basic keywords:
    ...
    -await:
        ...
        -When you call await, the function you're in gets suspended while whatever you asked to wait on happens, and
        then when it's finished, the event loop will wake the function up again and resume it from the await call,
        passing any result out.
-There’s also a strict set of rules around when and how you can and cannot use async/await.
    -A function that you introduce with async def is a coroutine. It may use await, return, or yield, but all of these
    are optional.
        -Using await and/or return creates a coroutine function.
        -To call a coroutine function, you must await it to get its results.
        -It is less common (and only recently legal in Python) to use yield in an async def block. This creates an
    ...
    -Just like it’s a SyntaxError to use yield outside of a def function, it is a SyntaxError to use await outside of
...
# 2) main() function pauses and gives back control to the event loop, which sees that get_chat_id needs to run.
# 3) Event loop calls get_chat_it and then that calls await and gets suspended with a marker to resume it in 3 seconds.
