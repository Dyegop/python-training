...
BASIC CONCEPTS:
    ...
    -Packages are organized hierarchically, and packages may themselves contain subpackages, as well as regular modules.
...
REGULAR PACKAGES:
    ...
    -Important concepts about __init__.py:
        -If you add imports clauses of classes and functions to the __init__.py file, they would be available as a top
        level object when people import your package.
        -The code in the __init__.py file is ran when you import the package. Think of it as the __init__ method inside
        a class.
        -Another common practice is to add some basic checks to the __init__.py file in order to make sure that
        the package works as expected.

NAMESPACE PACKAGES:
    -A namespace package is a composite of various portions, where each portion contributes a subpackage to the
    parent package.
    -Portions may reside in different locations on the file system, for example, in zip files, on the network,
    or anywhere else that Python searches during import.
...
__main__.py:
    ...
    -Example:
        ...
        bandclass
            ...
            ├── __main__.py
            └── student.py
        _main__.py will be executed when the package itself is invoked directly from the command line using the -m flag
