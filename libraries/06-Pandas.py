...
# reset_option(opt_name)      -> reset option to their default value. Use 'all' to reset all options
# display.max_rows    - change the maximum rows to be displayed. Select None to display all rows
# display.max_columns - change the maximum columns to be displayed. Select None to display all columns
...
print(df1[df1['W'] > 0][['Y', 'X']])         # Return rows from ['Y', 'X'] columns where condition is fulfilled
...
# concat() -> glue together DataFrames. Dimensions should match along the axis you are concatenating on
df_c1 = pd.DataFrame({'A': ['A0', 'A1', 'A2', 'A3'], 'B': ['B0', 'B1', 'B2', 'B3'],
                     'C': ['C0', 'C1', 'C2', 'C3'], 'D': ['D0', 'D1', 'D2', 'D3']}, index=[0, 1, 2, 3])
df_c2 = pd.DataFrame({'A': ['A4', 'A5', 'A6', 'A7'], 'B': ['B4', 'B5', 'B6', 'B7'],
                      'C': ['C4', 'C5', 'C6', 'C7'], 'D': ['D4', 'D5', 'D6', 'D7']}, index=[4, 5, 6, 7])
df_c3 = pd.DataFrame({'A': ['A8', 'A9', 'A10', 'A11'], 'B': ['B8', 'B9', 'B10', 'B11'],
...
df_excel = pd.read_excel('directory/file.xlsx', sheetname='Sheet1')
...
# Pandas allows you to visualize data based off of matplotlib plots
# Syntax to plot data -> dataframe.plot.plot_type(opt_parameters)
# Dataframe selections is allowed -> dataframe[['columnA', 'columnB']].plot.plot_type(opt_parameters)
...
# Area plots
df_csv2.plot.area(alpha=0.4)

# Barplots
df_csv2.plot.bar()
df_csv2.plot.bar(stacked=True)

# Histograms
df_csv1['A'].plot.hist(bins=50)

# Lineplots
df_csv1.plot.line(x=df1.index, y='B', figsize=(12, 3), lw=1)

# Scatter Plots
df_csv1.plot.scatter(x='A', y='B')
df_csv1.plot.scatter(x='A', y='B', c='C', cmap='coolwarm')
df_csv1.plot.scatter(x='A', y='B', s=df1['C']*200)

# Boxplots
df_csv2.plot.box()
df_csv2.plot.box(by='C')

# Hexagonal Bin Plot
df6 = pd.DataFrame(np.random.randn(1000, 2), columns=['a', 'b'])
df6.plot.hexbin(x='a', y='b', gridsize=25, cmap='Oranges')

# Kernel Density Estimation plot (KDE)
df_csv2['a'].plot.kde()
df_csv2.plot.density()
